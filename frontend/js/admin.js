...
function setupEvents() {
  ...
  // Initialize Theme Settings Listeners
...
function updateAdminLogos(theme) {
  ...
  if (loginLogo) {
    loginLogo.innerHTML = `<img src="${logoImg}" style="width: 100%; height: 100%; object-fit: cover;">`;
    loginLogo.style.background = 'none';
    loginLogo.style.borderRadius = '50%';
    loginLogo.style.boxShadow = '0 4px 12px rgba(0,0,0,0.15)';
  }

  // Update Sidebar Logo
  const logoIcon = document.querySelector('.logo-icon');
  if (logoIcon) {
    logoIcon.innerHTML = `<img src="${logoImg}" style="width: 100%; height: 100%; object-fit: cover;">`;
    logoIcon.style.background = 'none';
    logoIcon.style.borderRadius = '50%';
    logoIcon.style.boxShadow = '0 2px 8px rgba(0,0,0,0.1)';
  }

  // Update Media Preview Placeholder
  const photoInitials = document.getElementById('photoInitials');
  if (photoInitials) {
    photoInitials.innerHTML = `<img src="${logoImg}" style="width: 100%; height: 100%; object-fit: cover;">`;
    photoInitials.style.background = 'none';
    photoInitials.style.borderRadius = '50%';
    photoInitials.style.boxShadow = '0 10px 30px rgba(0,0,0,0.15)';
  }
}
  ...
  subscribeToAdminUpdates({
    ...
    onSettingsUpdate: (payload) => {
      ...
      if (settings) {
        ...
        // If we are on the settings page, refresh the fields
...
async function loadSettingsPage() {
  ...
  try {
    ...
    document.getElementById('settingSeoKeywords').value = currentSettings.seoKeywords || '';
    document.getElementById('settingSiteDescription').value = currentSettings.siteDescription || '';

    // Fill Theme Color
    const primaryColor = currentSettings.primaryColor || '#6366f1';
    document.getElementById('primaryColorPicker').value = primaryColor;
    document.getElementById('colorHex').textContent = primaryColor.toUpperCase();

    // Update presets active state
    updateColorPresetUI(primaryColor);
...
function updateColorPresetUI(activeColor) {
  const normColor = activeColor.toLowerCase();
  document.querySelectorAll('.color-preset').forEach(p => {
    if (p.dataset.color.toLowerCase() === normColor) {
...
function renderFavorites() {
  ...
  container.innerHTML = favoriteColors.map(color => `
    <div class="color-preset" data-color="${color}" style="background: ${color}; position: relative;" title="${color}" onclick="applyColorLocally('${color}')">
      <i class="fas fa-times-circle delete-fav" onclick="removeFavorite('${color}', event)" style="position: absolute; top: -5px; right: -5px; font-size: 14px; color: var(--error); cursor: pointer; background: white; border-radius: 50%; display: none;"></i>
    </div>
  `).join('');

  // Add hover logic manually for favorites
  container.querySelectorAll('.color-preset').forEach(p => {
    p.addEventListener('mouseenter', () => {
      const delIcon = p.querySelector('.delete-fav');
...
function saveCurrentAsFavorite() {
  const color = document.getElementById('primaryColorPicker').value.toUpperCase();
...
function removeFavorite(color, e) {
...
function applyColorLocally(color) {
  ...
  updateColorPresetUI(color);

  if (window.ThemeService) {
    window.ThemeService.applyThemeColor(color);
  }

  if (typeof sendThemeUpdate === 'function') {
    sendThemeUpdate({ primary_color: color });
  }
}
...
function initSettingsListeners() {
  // Built-in Presets
  document.querySelectorAll('#colorPresets .color-preset').forEach(p => {
    p.addEventListener('click', () => {
      applyColorLocally(p.dataset.color);
    });
  });
  ...
  if (picker) {
    picker.addEventListener('input', (e) => {
      applyColorLocally(e.target.value);
    });
  }
}

async function saveWebsiteSettings(retries = 2) {
  // Pass the actual button element
  const btn = document.querySelector('#settingsPage .card-header .btn-primary');
  if (!btn) return;
  ...
  const data = {
    ...
    seoKeywords: (document.getElementById('settingSeoKeywords')?.value || '').trim(),
    siteDescription: (document.getElementById('settingSiteDescription')?.value || '').trim(),
    primaryColor: (document.getElementById('primaryColorPicker')?.value || '#6366f1')
  };
  ...
  try {
    ...
    if (!res.ok) {
      if (retries > 0) {
        console.warn(`Retry saving... attempts left: ${retries}`);
        return saveWebsiteSettings(retries - 1);
      }
    ...
    // Refresh theme on page
    if (window.ThemeService) {
      window.ThemeService.applyThemeColor(data.primaryColor);
    }
...
window.applyColorLocally = applyColorLocally;

console.log('✅ Website Settings Module v2.0 Loaded');
